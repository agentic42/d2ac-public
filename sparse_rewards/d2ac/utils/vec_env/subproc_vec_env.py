...
from d2ac.utils.vec_env import CloudpickleWrapper, VecEnv
...
def _worker(remote, parent_remote, env_fn_wrapper):
    ...
    while True:
        try:
            ...
            if cmd == "step":
                observation, reward, done, _, info = env.step(data)
                """
                if done:
...
class SubprocVecEnv(VecEnv):
    ...
    def __init__(self, env_fns):
        ...
        if isinstance(observation_space, spaces.Dict):
            ...
            if (
                hasattr(dummy_env, "goal_extraction_function")
                and dummy_env.goal_extraction_function is not None
            ):
    ...
    def reset(self):
        ...
        if self.goal_env:
            obs = {k: np.stack([o[0][k] for o in obs]) for k in self.goal_keys}
        else:
            obs = np.stack(obs)
        return obs
